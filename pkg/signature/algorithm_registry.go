...
// GetDefaultAlgorithmDetailsForPublicKey returns the default algorithm details for a given public key.
// This is useful for cases where a public key is not associated with a specific signature algorithm,
// such as when verifying a signature.
//
// The idea is to have one default algorithm for a given public key type, so that just using the public key
// you can determine the right algorithm to use.
func GetDefaultAlgorithmDetailsForPublicKey(pubKey crypto.PublicKey) (AlgorithmDetails, error) {
	switch pk := pubKey.(type) {
	case *rsa.PublicKey:
		switch pk.Size() * 8 {
		case 2048:
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_RSA_PKCS1V15_2048_SHA256)
		case 3072:
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_RSA_PKCS1V15_3072_SHA256)
		case 4096:
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_RSA_PKCS1V15_4096_SHA256)
		}
	case *ecdsa.PublicKey:
		switch pk.Curve {
		case elliptic.P256():
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_ECDSA_P256_SHA_256)
		case elliptic.P384():
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_ECDSA_P384_SHA_384)
		case elliptic.P521():
			return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_ECDSA_P521_SHA_512)
		default:
			return nil, fmt.Errorf("unsupported ECDSA curve: %s", pk.Curve.Params().Name)
		}
	case ed25519.PublicKey:
		return getAlgorithmDetails(v1.PublicKeyDetails_PKIX_ED25519_PH)
	}
