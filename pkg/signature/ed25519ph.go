...
// ED25519phSigner is a signature.Signer that uses the Ed25519 public-key signature system
...
func (e ED25519phSigner) SignMessage(message io.Reader, opts ...SignOption) ([]byte, error) {
	...
	return e.priv.Sign(nil, digest, &ed25519.Options{Hash: crypto.SHA512})
}
...
// Sign computes the signature for the specified digest.
//
...
func (e ED25519phSigner) Sign(_ io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	ed25519phOpts := []SignOption{options.WithDigest(digest)}
	if opts != nil {
		ed25519phOpts = append(ed25519phOpts, options.WithCryptoSignerOpts(opts))
	}

	return e.SignMessage(nil, ed25519phOpts...)
}
